...
import { FACT_STORAGE_LIMIT } from '../constants/facts';
import { HttpError } from '../helpers/error';
import { configureFactItems } from '../helpers/facts';
import { isReqValid } from '../helpers/http';
import logger from '../helpers/logger';
import FactModel from '../models/fact';
import UserModel from '../models/user';
import { TFactItem } from '../types/fact';
...
export const getFacts = async (
  ...
  try {
    ...
    const facts = await FactModel.find().skip(+offset).limit(+limit).lean();
    ...
    const factItems: TFactItem[] = facts.map(({ _id, ...data }) => ({
      id: _id.toString(),
      ...data,
    }));
